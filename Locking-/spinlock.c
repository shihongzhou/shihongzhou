...
{									\
	for (;;) {							\
		...
		arch_##op##_relax(&lock->raw_lock);			\
									...
									\
/*							*/	\
/* __raw_##op##_lock_irqsave - Acquire a raw lock with IRQs disabled, saving interrupt flags  */	\
/* @lock: Pointer to the locktype##_t structure representing the raw lock (e.g., spinlock_t or rwlock_t) */	\
/* */	\
/* This function acquires a raw lock (e.g., spinlock or rwlock) while disabling */	\
/* interrupts to ensure atomicity and prevent deadlocks in critical sections.*/	\
/* It saves the current state of interrupt flags and returns them, so they can */	\
/* be restored later when the lock is released.*/	\
/**/	\
/* The function is implemented as a loop to ensure that the lock is acquired */	\
/* even if there is contention, and it uses a "relax" mechanism to reduce CPU */	\
/* load while waiting for the lock.*/	\
/* */	\
/* The function is marked with the `__lockfunc` attribute to indicate that it belongs */	\
/* to the lock function section, allowing special handling by the kernel's lock tracing */	\
/* and debugging mechanisms.*/	\
/**/	\
/* This macro expands into a function definition with a name that incorporates the */	\
/* specific lock operation (`op`), such as `__raw_spin_lock_irqsave` or */	\
/* `__raw_rwlock_irqsave`, depending on the type of lock being acquired.*/	\
/*							*/	\
...
/**
 * in_lock_functions - Check if an address belongs to the lock functions section
 * @addr: The address to check
 *
 * This function determines whether a given address (`addr`) falls within the 
 * range of addresses designated for lock-related functions. The range is defined 
 * by the `__lock_text_start` and `__lock_text_end` symbols, which are provided 
 * by the linker and mark the start and end of the section containing lock functions.
 *
 * This function is marked `notrace` to prevent it from being traced by tracing 
 * facilities (e.g., ftrace), as it is often used in contexts where tracing itself 
 * could interfere with lock function detection.
 *
